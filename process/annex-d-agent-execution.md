...
**The Jira key is the index.** Every developer activity has a ticket, so the ticket key names the
folder, `work/README.md` (auto-generated by `tools/update-index.sh` on every scaffold/save) lists
every workspace with its status/phase, and the Phase 0 ticket comment carries the workspace URL —
Jira → folder and folder → Jira, both directions always resolvable.
